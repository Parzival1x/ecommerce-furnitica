...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			if(count($_FILES["image_gallery"]["name"]) > 0){
				for($i=0;$i<count($_FILES["image_gallery"]["name"]);$i++) {
					$g_image=rand(111111111,999999999).'_'.$_FILES["image_gallery"]["name"][$i];
					move_uploaded_file($_FILES['image_gallery']['tmp_name'][$i],PRODUCT_IMAGE_SERVER_PATH.$g_image);
					$created_at=date('Y-m-d H:i:s');
					mysqli_query($con,"insert into product_img(product_id,image_url,created_at) values('$last_id','$g_image','$created_at')");
				}
			}
		}
		header('location:product.php');
				...
				<form method="post" enctype="multipart/form-data">
					<div class="card-body card-block">
						...
						<div class="form-group">
							...
							<input type="file" name="image" class="form-control" <?php echo  $image_required?> accept="image/*">
						</div>
						<div class="form-group">
							<label for="categories" class=" form-control-label">Image Gallery</label>
							<input type="file" name="image_gallery[]" class="form-control" multiple accept="image/*">
						</div>
						<div class="form-group">
